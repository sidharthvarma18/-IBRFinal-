...
st.set_page_config(page_title="IBR Dashboard | Credit Card Analysis",
...
.dash-header{background:linear-gradient(135deg,#0D1B4B 0%,#1A3C8F 60%,#2E8B9A 100%);
  padding:1.8rem 2.5rem 1.4rem;border-radius:16px;margin-bottom:1.5rem;}
.dash-header h1{font-size:1.75rem;font-weight:700;margin:0;color:white;}
.dash-header p{font-size:0.9rem;color:#CCE0FF;margin:.3rem 0 0;}
.badge{display:inline-block;background:rgba(232,168,56,.2);border:1px solid #E8A838;
  color:#E8A838;padding:.15rem .65rem;border-radius:20px;font-size:.72rem;font-weight:600;margin-right:.4rem;}
...
.kpi-card.teal{border-left-color:#2E8B9A;} .kpi-card.gold{border-left-color:#E8A838;}
.kpi-lbl{font-size:.72rem;color:#000;font-weight:700;text-transform:uppercase;letter-spacing:.05em;}
.kpi-val{font-size:1.55rem;font-weight:700;color:#000;line-height:1.2;}
.kpi-sub{font-size:.72rem;color:#333;margin-top:.15rem;}
...
.insight{background:#EBF4FF;border-left:4px solid #1A3C8F;
  padding:.75rem 1rem;border-radius:0 8px 8px 0;font-size:.83rem;color:#000;margin-top:.5rem;}
...
.stTabs [data-baseweb="tab"]{background:white;border-radius:8px 8px 0 0;
  padding:.4rem 1.1rem;font-weight:600;color:#000;}
...
BK = "#000000"  # universal black for all chart text

def rgba(hex_col, alpha=0.2):
    h=hex_col.lstrip("#"); r,g,b=int(h[0:2],16),int(h[2:4],16),int(h[4:6],16)
    return f"rgba({r},{g},{b},{alpha})"

# font dicts reused everywhere
FT  = dict(color=BK, family="Inter")          # general font
AFT = dict(color=BK, family="Inter", size=12) # axis tick font
LFT = dict(color=BK, family="Inter", size=12) # legend font
XAX = dict(showgrid=False, tickfont=AFT, title_font=FT, color=BK)
YAX = dict(showgrid=True, gridcolor="#F0F4FA", tickfont=AFT, title_font=FT, color=BK)
LEG = dict(orientation="h", yanchor="bottom", y=-0.35, xanchor="left", font=LFT)

def wl(fig, h=360, angle=0):
    fig.update_layout(plot_bgcolor="white", paper_bgcolor="white",
        font=FT, height=h, xaxis=XAX, yaxis=YAX, legend=LEG, margin=dict(b=80))
    if angle:
        fig.update_xaxes(tickangle=angle, tickfont=AFT)
    return fig
...
fi=fdf[fdf["Country"]=="India"]; fu=fdf[fdf["Country"]=="UAE"]

st.markdown("""<div class="dash-header">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:1rem">
    <div><h1>💳 Credit Card Market Analysis Dashboard</h1>
    ...
    <div style="text-align:right;color:#CCE0FF;font-size:.82rem">
      <div>Mentor: Ms. Farah Naaz</div>
      <div style="margin-top:.3rem">Pearson r · OLS Regression · t-test · Global Benchmarking</div>
    </div></div></div>""", unsafe_allow_html=True)

k=st.columns(6)
kpis=[("Total Spends",f"₹{fdf['Annual_Spends_INR_Cr'].sum()/1e5:.1f}L Cr","Aggregate INR Crores",""),
...
def tl(fig, xv, yv, col):
    mask=~(np.isnan(xv)|np.isnan(yv))
    if mask.sum()<3: return fig
    sl,ic,_,_,_=stats.linregress(xv[mask],yv[mask])
...
t1,t2,t3,t4,t5,t6,t7=st.tabs(["📈 Spending Trends","📊 Descriptive Stats","🔗 Correlation",
...
with t1:
    ...
    with c1:
        if "India" in sel_c and len(fi):
            ...
            fig.update_layout(plot_bgcolor="white",paper_bgcolor="white",font=FT,height=360,
                xaxis=dict(**XAX,dtick=1),yaxis=YAX,
                legend=dict(**LEG,title_font=FT),margin=dict(b=80))
    ...
    with c2:
        if "UAE" in sel_c and len(fu):
            ...
            fig2.update_layout(plot_bgcolor="white",paper_bgcolor="white",font=FT,height=360,
                xaxis=dict(**XAX,dtick=1),yaxis=YAX,
                legend=dict(**LEG,title_font=FT),margin=dict(b=80))
    ...
    fh=px.imshow(ht,color_continuous_scale=[[0,"#E84A38"],[.5,"#FFFFFF"],[1,"#1A3C8F"]],
        color_continuous_midpoint=0,text_auto=".1f",title="YoY Growth Rate (%) — All Banks",aspect="auto")
    fh.update_layout(font=FT,height=380,plot_bgcolor="white",paper_bgcolor="white",
        xaxis=dict(tickfont=AFT,title_font=FT),yaxis=dict(tickfont=AFT,title_font=FT),
        coloraxis_colorbar=dict(tickfont=AFT,title_font=FT))
    fh.update_traces(textfont=dict(color=BK,size=10))
    ...
    fa.update_layout(plot_bgcolor="white",paper_bgcolor="white",font=FT,height=300,
        xaxis=dict(**XAX,dtick=1),yaxis=dict(**YAX,tickformat=",.0f"),
        legend=dict(orientation="h",y=-0.25,font=LFT))
...
with t2:
    ...
    with c1:
        fb=px.box(ds,x="Bank",y="Annual_Spends_INR_Cr",color="Country",
            ...
            color_discrete_map={"India":BLUE,"UAE":TEAL})
        fb.update_layout(plot_bgcolor="white",paper_bgcolor="white",font=FT,height=380,
            xaxis=dict(**XAX,tickangle=-35),yaxis=YAX,legend=dict(orientation="h",y=-0.35,font=LFT))
    ...
    with c2:
        ...
        fhi.update_layout(plot_bgcolor="white",paper_bgcolor="white",font=FT,height=380,
            xaxis=dict(**XAX),yaxis=YAX,legend=dict(orientation="h",y=-0.25,font=LFT))
    ...
    with c1:
        fsk=px.bar(skdf,x="Variable",y="Skewness",title="Skewness (>1 = highly right-skewed)",
            color="Skewness",color_continuous_scale=[[0,"#2E8B9A"],[.5,"white"],[1,"#1A3C8F"]])
        fsk.add_hline(y=1,line_dash="dash",line_color=GOLD,annotation_text="Skewness=1",
            annotation_font=dict(color=BK))
        fsk.update_layout(plot_bgcolor="white",paper_bgcolor="white",font=FT,height=320,
            xaxis=dict(**XAX,tickangle=-20),yaxis=YAX,
            coloraxis_colorbar=dict(tickfont=AFT))
        st.plotly_chart(fsk,use_container_width=True)
    with c2:
        fku=px.bar(skdf,x="Variable",y="Kurtosis",title="Kurtosis (>3 = leptokurtic)",
            color="Kurtosis",color_continuous_scale=[[0,"#2E8B9A"],[.5,"white"],[1,"#1A3C8F"]])
        fku.add_hline(y=3,line_dash="dash",line_color=RED,annotation_text="Normal=3",
            annotation_font=dict(color=BK))
        fku.update_layout(plot_bgcolor="white",paper_bgcolor="white",font=FT,height=320,
            xaxis=dict(**XAX,tickangle=-20),yaxis=YAX,
            coloraxis_colorbar=dict(tickfont=AFT))
...
with t3:
    ...
    fcm=go.Figure(go.Heatmap(z=cm.values,x=cm.columns,y=cm.index,
        colorscale=[[0,"#E84A38"],[.5,"#FFFFFF"],[1,"#1A3C8F"]],zmid=0,zmin=-1,zmax=1,
        text=cm.values,texttemplate="%{text:.2f}",textfont=dict(color=BK,size=12),
        hovertemplate="<b>%{y} vs %{x}</b><br>r=%{z:.3f}<extra></extra>"))
    fcm.update_layout(title=f"Pearson Correlation Matrix — {cmkt}",font=FT,height=420,
    ...
    with c1:
        ...
        fs1=tl(fs1,idf["Interest_Rate_Pct"].values,idf["Annual_Spends_INR_Cr"].values,BLUE)
        fs1=tl(fs1,udf["Interest_Rate_Pct"].values,udf["Annual_Spends_INR_Cr"].values,TEAL)
        fs1.update_layout(plot_bgcolor="white",paper_bgcolor="white",font=FT,height=370,
            xaxis=XAX,yaxis=YAX,legend=dict(orientation="h",y=-0.3,font=LFT))
        ...
        <b>UAE:</b> r={ru:.3f}, p={pu:.4f} — {'✅ Significant (premium product-mix effect)' if pu<0.05 else '❌ NS'}
    ...
    with c2:
        fs2=px.scatter(df,x="Fee_Commission_INR_Cr",y="Annual_Spends_INR_Cr",color="Country",
            ...
            color_discrete_map={"India":BLUE,"UAE":TEAL},hover_data=["Bank","Year"])
        fs2=tl(fs2,idf["Fee_Commission_INR_Cr"].values,idf["Annual_Spends_INR_Cr"].values,BLUE)
        fs2=tl(fs2,udf["Fee_Commission_INR_Cr"].values,udf["Annual_Spends_INR_Cr"].values,TEAL)
        fs2.update_layout(plot_bgcolor="white",paper_bgcolor="white",font=FT,height=370,
            xaxis=XAX,yaxis=YAX,legend=dict(orientation="h",y=-0.3,font=LFT))
    ...
    frr.add_hline(y=20,line_dash="dash",line_color=GOLD,annotation_text="20% threshold",
        annotation_font=dict(color=BK))
    frr.update_layout(plot_bgcolor="white",paper_bgcolor="white",font=FT,height=320,
        xaxis=dict(**XAX,dtick=1),yaxis=YAX,
        legend=dict(orientation="h",y=-0.25,font=LFT),hovermode="x unified")
...
with t4:
    ...
    for ctry,cd in [("India",fi),("UAE",fu)]:
        ...
        xd,yd=cd[rx].values,cd[ry].values; m=~(np.isnan(xd)|np.isnan(yd))
        if m.sum()<3: continue
    ...
    if res: st.dataframe(pd.DataFrame(res).set_index("Country"),use_container_width=True)
    ...
    for i,(ctry,cd,pal) in enumerate([("India",fi,BLUE),("UAE",fu,TEAL)]):
        ...
        with [c1,c2][i]:
            fr=px.scatter(cd,x=rx,y=ry,color="Bank",
                ...
                color_discrete_sequence=IP if ctry=="India" else UP,hover_data=["Bank","Year"])
            xd,yd=cd[rx].values,cd[ry].values; m=~(np.isnan(xd)|np.isnan(yd))
            if m.sum()>2:
            ...
            fr.update_layout(plot_bgcolor="white",paper_bgcolor="white",font=FT,height=370,
                xaxis=XAX,yaxis=YAX,legend=dict(orientation="h",y=-0.38,font=LFT))
    ...
    for ctry,cd in [("India",idf),("UAE",udf)]:
        for yv,xv in pairs:
            xd,yd=cd[xv].values,cd[yv].values; m=~(np.isnan(xd)|np.isnan(yd))
            if m.sum()>2:
...
with t5:
    ...
    with c1:
        ...
        fra.update_layout(paper_bgcolor="white",font=FT,height=420,
            polar=dict(radialaxis=dict(visible=True,range=[0,1],tickfont=AFT),
                       angularaxis=dict(tickfont=AFT)),
            showlegend=True,legend=dict(orientation="h",y=-0.1,font=LFT),
            title=dict(text="Normalised Market Profile",font=FT))
    ...
    ffy=px.line(fyr,x="Year",y="Pct",color="Country",
        ...
        color_discrete_map={"India":BLUE,"UAE":TEAL},markers=True)
    ffy.update_layout(plot_bgcolor="white",paper_bgcolor="white",font=FT,height=320,
        xaxis=dict(**XAX,dtick=1),yaxis=YAX,
        legend=dict(orientation="h",y=-0.25,font=LFT),hovermode="x unified")
...
with t6:
    ...
    bdf=df[df["Bank"]==sb].copy(); ctry=bdf["Country"].iloc[0]
    ...
    with c1:
        ...
        fd1.update_layout(title=f"{sb} — Spends & Interest Rate",plot_bgcolor="white",paper_bgcolor="white",
            font=FT,height=360,legend=dict(orientation="h",y=-0.28,font=LFT),
            xaxis=dict(tickfont=AFT),yaxis=dict(tickfont=AFT),yaxis2=dict(tickfont=AFT))
    ...
    with c1:
        fl1=px.bar(latest.sort_values("Annual_Spends_INR_Cr",ascending=False),x="Bank",
            ...
            color_discrete_map={"India":BLUE,"UAE":TEAL})
        fl1.update_layout(plot_bgcolor="white",paper_bgcolor="white",font=FT,height=340,
            xaxis=dict(**XAX,tickangle=-30),yaxis=YAX,legend=dict(orientation="h",y=-0.35,font=LFT))
    ...
    with c2:
        fl2=px.bar(latest.sort_values("Interest_Rate_Pct",ascending=False),x="Bank",
            ...
            color_discrete_map={"India":BLUE,"UAE":TEAL})
        fl2.update_layout(plot_bgcolor="white",paper_bgcolor="white",font=FT,height=340,
            xaxis=dict(**XAX,tickangle=-30),yaxis=YAX,legend=dict(orientation="h",y=-0.35,font=LFT))
...
with t7:
    ...
    with c1:
        ...
        fb1.update_layout(plot_bgcolor="white",paper_bgcolor="white",font=FT,height=330,
            showlegend=False,xaxis=XAX,yaxis=YAX)
    ...
    with c2:
        ...
        fb2.update_layout(plot_bgcolor="white",paper_bgcolor="white",font=FT,height=330,
            showlegend=False,xaxis=XAX,yaxis=YAX)
    ...
    frr.update_layout(paper_bgcolor="white",font=FT,height=480,
        polar=dict(radialaxis=dict(visible=True,range=[0,1],tickfont=AFT),
                   angularaxis=dict(tickfont=AFT)),
        showlegend=True,legend=dict(orientation="h",y=-0.12,font=LFT),
        title=dict(text="Multi-Dimensional Market Profile — Normalised",font=FT))
    st.plotly_chart(frr,use_container_width=True)
    ...
    fee revenue per cardholder falls short of covering 8–12 annual visits. UAE is structurally viable
    at ~5% fee yields. India must implement spend-threshold access gates (Singapore MAS model).</div>""",
    unsafe_allow_html=True)

st.markdown("<br><br>",unsafe_allow_html=True)
st.markdown("""<div style="background:#0D1B4B;padding:1rem 2rem;border-radius:12px;color:#CCE0FF;
